...
module SE_sampler (
    ...
    generate
        for (i = 0; i < 4; i = i + 1) begin : sampler_gen
            ...
            // 模数处理：1344/976 使用 q=2^16 (mask FFFF)，640 使用 q=2^15 (mask 7FFF)
                ...
                case (frodo_mode)
                    2'b00: begin // 640
                        for (int j = 0; j < 13; j = j + 1) begin
                            if (z >= CDT_640[j][14:0]) e_val = e_val + 1;
                        end
                    end
                    2'b01: begin // 976
                        for (int j = 0; j < 11; j = j + 1) begin
                            if (z >= CDT_976[j][14:0]) e_val = e_val + 1;
                        end
                    end
                    2'b10: begin // 1344
                        for (int j = 0; j < 7; j = j + 1) begin
                            if (z >= CDT_1344[j][14:0]) e_val = e_val + 1;
                        end
                    end
                    default: ;
                endcase

                // 如果符号位 r0 为 1 且采样值不为 0，则取负 (e = -e mod q)
                if (r0 == 1'b1 && e_val != 16'd0) begin
                    e_val = (~e_val + 16'd1) & mod_q_mask;
                end else begin
                    e_val = e_val & mod_q_mask;
                end
            end
            assign se_data[i*16 +: 16] = e_val;
        end
    endgenerate
endmodule
